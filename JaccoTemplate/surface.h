...
constexpr unsigned int GreenMask = 0x00ff00;
constexpr unsigned int BlueMask = 0x0000ff;

typedef unsigned int Pixel; // unsigned int is assumed to be 32-bit, which seems a safe assumption.
static_assert(sizeof(Pixel) == 4, "Pixel datatype is expected to be 4 bytes (32-bits)"); //Still a good assumption to double check.
...
public:
	...
	void SetFlags(Flags a_Flags ) { m_Flags = a_Flags; }
